...
template <size_t MaxSubscriptions = Default_Subscriptions_Amount, size_t MaxAttributes = Default_Attributes_Amount,
...
#if THINGSBOARD_ENABLE_DYNAMIC
        auto it = std::find_if(m_attribute_request_callbacks.begin(), m_attribute_request_callbacks.end(),
                               [&request_id](Attribute_Request_Callback& attribute_request) {
#else
        auto it = std::find_if(m_attribute_request_callbacks.begin(), m_attribute_request_callbacks.end(),
                               [&request_id](Attribute_Request_Callback<MaxAttributes>& attribute_request)
                               {
#endif // THINGSBOARD_ENABLE_DYNAMIC
...
private:
    ...
    /// @return Whether requesting the given callback was successful or not
#if THINGSBOARD_ENABLE_DYNAMIC
    bool Attributes_Request(Attribute_Request_Callback const& callback, char const* attribute_request_key,
                            char const* attribute_response_key)
    {

#else
    // NIMA CHANGES: fixed a bug in this method - removed trailing comma
    bool Attributes_Request(Attribute_Request_Callback<MaxAttributes> const& callback,
                            ...
                            char const* attribute_response_key)
    {
        ...
        // --- Size calculation without trailing comma ---
        size_t size = 0U;
        bool first = true;
        ...
        char request[size];
        request[0] = '\0'; // ensure empty C string

        // --- Build comma-separated string without trailing comma ---
        first = true;
        for (const auto& att : attributes)
        {
            if (Helper::stringIsNullorEmpty(att))
            ...
            if (!first)
            {
                strncat(request, ",", size - strlen(request) - 1);
            }
            strncat(request, att, size - strlen(request) - 1);
            first = false;
        }
        ...
        // NIMA CHANGES
        request_buffer["deviceName"] = GetDeviceId();
    ...
    /// @return Whether requesting the given callback was successful or not
#if THINGSBOARD_ENABLE_DYNAMIC
    bool Attributes_Request_Subscribe(Attribute_Request_Callback const& callback,
                                      Attribute_Request_Callback* & registered_callback)
    {

#else
    bool Attributes_Request_Subscribe(Attribute_Request_Callback<MaxAttributes> const& callback,
                                      Attribute_Request_Callback<MaxAttributes>* & registered_callback)
    {
#endif // THINGSBOARD_ENABLE_DYNAMIC
#if !THINGSBOARD_ENABLE_DYNAMIC
    ...
    // Client-side or shared attribute request callback vector
#else
    Array<Attribute_Request_Callback<MaxAttributes>, MaxSubscriptions> m_attribute_request_callbacks = {};
    // Client-side or shared attribute request callback array
#endif // THINGSBOARD_ENABLE_DYNAMIC
};
